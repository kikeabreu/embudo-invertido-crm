...
export default function BancoTab({ piezas = [], onSave, onAdd, onImport, onDelete, onBulkDelete, onBulkUpdate, isViewer, canEdit, canDelete, canImport, logs = [], toast, userRole, brokerId, onCreateTarea, currentUser }) {
    ...
    return (
        <div style={{ padding: "24px 28px", overflowY: "auto", height: "100%", boxSizing: "border-box" }}>
            {editPiece && <PieceModal piece={editPiece} isViewer={isViewer} canEdit={canEdit} canDelete={canDelete} userRole={userRole} onSave={p => { if (canEdit) { onSave(p); setEditPiece(null); } }} onClose={() => setEditPiece(null)} onDelete={canDelete ? (id => { onDelete(id); setEditPiece(null); }) : null} logs={logs} toast={toast} brokerId={brokerId} currentUser={currentUser} onCreateTarea={onCreateTarea} />}
